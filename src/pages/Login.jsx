...
export default function Login() {
  ...
  // Redirect once currentUser is set — avoids race condition
  useEffect(() => {
    if (!currentUser) return;
    console.log("[Login] Redirecting for role:", currentUser.role);
    if (currentUser.role === "student")  navigate("/dashboard",    { replace: true });
    else if (currentUser.role === "faculty") navigate("/faculty",  { replace: true });
    else if (currentUser.role === "admin")   navigate("/admin/users", { replace: true });
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="bg-navy-800 border border-navy-600 rounded-2xl p-8">
            ...
            <p className="text-slate-400 text-sm mb-6">
              An OTP has been sent to <span className="text-gold-400">{email}</span>
            </p>
        ...
        <div className="bg-navy-800 border border-navy-600 rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <button type="button" onClick={() => setShowOtp(true)} className="text-xs text-gold-400 hover:text-gold-300 transition-colors">
                  Forgot Password?
                </button>
              </div>
              <div className="relative">
          ...
          <div className="mt-4 pt-4 border-t border-navy-600">
            <button className="w-full border border-navy-500 hover:border-gold-500/50 text-slate-300 hover:text-white font-medium py-2.5 rounded-lg text-sm transition-colors flex items-center justify-center gap-2">
              <GraduationCap size={15} /> Faculty Sign In
            </button>
          </div>
        ...
        <div className="mt-4 bg-navy-800/50 border border-navy-600/50 rounded-xl p-4">
          ...
          <div className="grid grid-cols-3 gap-2 text-xs text-center">
            {[
              ...
              <button key={role} onClick={() => { setEmail(em); setPassword("Test@1234"); }}
